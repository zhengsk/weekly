<%	
	...
	if ID=firstID then													'如果已经是第一篇文章
		sql="select top 1 ID,title from news where ID<"&ID&" order by ID desc" 		'获取下一篇文章的标题和ID
		rs.open sql,conn,0,1
		nexttitle="<a href=""admin-news.asp?ID="&rs("ID")&""">"&rs("title")&"</a>"
		rs.close
		previewtitle="<span>已经是第一篇</span>"
	elseif ID=lastID then 												'如果已经最后篇文章
		sql="select top 1 ID,title from news where ID>"&ID&" order by ID asc" 		'获取上一篇文章的标题
		rs.open sql,conn,0,1
		previewtitle="<a href=""admin-news.asp?ID="&rs("ID")&""">"&rs("title")&"</a>"
		rs.close
		...
		previewtitle="<a href=""admin-news.asp?ID="&rs("ID")&""">"&rs("title")&"</a>"
		rs.close
		sql="select top 1 ID,title from news where ID<"&ID&" order by ID desc"  		'获取下一篇文章的标题
		rs.open sql,conn,0,1
		nexttitle="<a href=""admin-news.asp?ID="&rs("ID")&""">"&rs("title")&"</a>"
		rs.close
	end if 
