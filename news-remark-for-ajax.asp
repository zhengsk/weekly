...
<% 
	...
	set rsmark02=server.CreateObject("adodb.recordset")
		sql="select top 5 [remark_name],[remark],[face],[tim] from [news_remark] where ID="&ID&" order by ord desc "
		sql02="select count(face) from [news_remark] where ID="&ID
	if showall="true" then sql="select [remark_name],[remark],[face],[tim] from [news_remark] where ID="&ID&" order by ord desc "
