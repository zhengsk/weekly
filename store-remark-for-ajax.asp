...
<% 
	...
	set rsmark02=server.CreateObject("adodb.recordset")
		sql="select top 5 [remark_name],[remark],[face],[tim] from [store_remark] where ID="&ID&" order by ord desc "
		sql02="select count(face) from [store_remark] where ID="&ID
	if showall="true" then sql="select [remark_name],[remark],[face],[tim] from [store_remark] where ID="&ID&" order by ord desc "
	...
	for i=1 to scorecount
		...
		response.Write("</cite>位观看者:<span>"&rsmark("remark_name")&"</span>"&rsmark("tim")&"</td></tr><tr><td><p><b><img src=""image/face"&rsmark("face")&".gif"" alt=""支持"" />评论:</b>"&rsmark("remark").value&"</p></td></tr>")
	rsmark.moveprevious
