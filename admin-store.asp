...
// JavaScript Document
  ...
  function mainAjax()		//对象上的函数 , 为主要的Ajax方法.
  {
	 if(showall=="false")
	  ...
	  var url="admin-store-remark-for-ajax.asp?id="+ theID +"&showall="+ showall
...
if ID="" then 
	sql="select top 1 [ID],[title],[contant] from store order by id desc"
else
	sql="select [ID],[title],[contant] from store where ID="&ID
end if 
rs.open sql,conn,0,1
dim ID,contant
ID=rs("ID")
storetitle=rs("title")
storecontant=rs("contant")
rs.close
%>
...
<div class="main">
  <div class="left">
        <p class="modifyAll">
        <a href="admin/store-modify.asp?id=<%=ID%>" target="_blank">修改当前页面</a>
        </p>
		<h5><%=storetitle%></h5><!------------------------------------------------------------商店名称--------->
		<div class="article"><%=storecontant%></div>
        <div class="pre-nex"> <!--#include file="admin-store-pre-next.asp"--> </div><!--------------------------------------------上下导航--------->
   	<div class="remark" id="remark">
   		<!--#include file="admin-store-remark.asp"--> <!--------------------------------------------留言内容------------------------>	
   	</div>
    <div class="remarkform">
		<form action="store-check-leaveword.asp?ID=<%=ID%>" name="msg" id="msg" method="post">
    ...
    <a class="toallimg" href="store-showall.asp" title="查看更多" id="toall" onfocus="this.blur()"><div></div></a>
