...
window.onload=ifUptheImg;
</script>
<% 
	theId=request.QueryString("id")
	sql="select * from store where id="&theId
	set rs=conn.execute(sql)
	title=rs("title")
	home_img=rs("home_img")
	s_contant=rs("s_contant")
	contant=rs("contant")
%>
</head>
...
<h1>商家推荐----修改</h1>
<div id="wrapper">
<form action="store-modify-save.asp?id=<%=response.Write(theId)%>" method="post" target="_blank" name="myform">

<label for="title">商家名称</label><input name="title" type="text" size="50" maxlength="50" value="<%=title%>"/>
<hr />

<div class="authorInfor">
	<label for="author">商家简介(显示在首页)及图片:</label>
	<textarea name="s_contant" cols="40" rows="10" ><%=s_contant%></textarea>
	<img src="../image/nopeole.gif" width="250" height="150" id="peoImg" title="显示在首页的图片" />
	<div>
	  <input type=text name='imgin' id="imgin" size=50 onchange="showright('peoImg','imgin')" value="<%=home_img%>" />
	  <input type=button id="toUpImg" value="首页图片..." onclick="showUploadDialog('image', 'myform.imgin', '')" />
