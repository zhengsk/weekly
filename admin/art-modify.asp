...
<h1>原创文学添加</h1>
<div id="wrapper">
<form action="art-modify-save.asp?articleId=<%=articleId%>" method="post" target="_blank" name="myform">

<label for="title">文章题目</label><input name="title" value="<%=title%>" type="text" size="50" maxlength="50"/>
<label for="authorname">作者姓名：</label><input name="authorname" value="<%=author%>" type="text" size="50" maxlength="50"/>
<hr />

<div class="authorInfor">
	<label for="author">作者简介：</label><textarea name="author" cols="40" rows="10" ><%=author_info%></textarea>
	<img src="../image/nopeole.gif" width="250" height="150" id="peoImg" />
	<div>
		<input type=text name='imgin' value="<%=author_img%>" id="imgin" size=50 onblur="showright('peoImg','imgin')">
        <input type=button id="toUpImg" value="上传图片..." onclick="showUploadDialog('image', 'myform.imgin', '')" />
...
<hr />
<label for="s_contant">内容简介：</label><textarea name="s_contant" cols="60" rows="10" ><%=s_contant%></textarea>
	<hr />
...
<br />
<input name="ok" type="submit" value="修改完成""/>
</form>
</div>
</body>
</html>
