...
<% 
	...
	set rsmark02=server.CreateObject("adodb.recordset")
		sql="select top 5 [ord],[remark_name],[remark],[heart],[face],[time] from [peo_remark] where week="&week&" order by ord desc "
		sql02="select remarknum,heart,facex,facey,facez from [people] where week="&week
	if showall="true" then sql="select  [ord],[remark_name],[remark],[heart],[face],[time]from [peo_remark] where week="&week&" order by ord desc "
	...
	'显示--------是否显示所有留言
		if showall="true" then 
			response.write("<div class=""showall""><a name=""forAjax"">取消显示所有留言</a></div>")
		else
			if remarknum>5 then
	...
	for i=1 to remarknum				
		...
		response.Write("</cite>位评委:<span>"&rsmark("remark_name")&"</span>"&rsmark("time")&"</td><td><em>评分:<span>"&showscore(rsmark("heart"))&"</span></em></td><tr><td colspan=""2"" ><p><img src=""image/"&rsmark("face")&".jpg"" alt=""支持"" /><b>评:</b>"&rsmark("remark").value&"</p><p class=""modifyThisRemark""><a href=""admin/peo-remark-modify.asp?ord="&rsmark("ord")&""" onclick=""modifyOrDelete(this);return false"" >修改</a><a href=""admin/peo-remark-delete.asp?ord="&rsmark("ord")&""" onclick=""modifyOrDelete(this);return false"" >删除</a></p></td></tr>")
	rsmark.moveprevious
	...
	'显示--------是否显示所有留言
		if showall="true" then 
			response.write("<div class=""showall""><a  href=""#remark"" name=""forAjax"">取消显示所有留言</a></div>")
		else
			if remarknum>5 then
