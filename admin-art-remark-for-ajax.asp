...
<% 
	...
	set rsmark02=server.CreateObject("adodb.recordset")
		sql="select top 5 [ord],[remark_name],[remark],[score] from [art_remark] where ID="&ID&" order by ord desc "
		sql02="select sum(score)  ,avg(score) ,count(score) from [art_remark] where ID="&ID
	if showall="true" then sql="select  [ord],[remark_name],[remark],[score] from [art_remark] where ID="&ID&" order by ord desc "
	...
	for i=1 to scorecount
		...
		response.Write("</cite>位评委:<span>"&rsmark("remark_name")&"</span></td><td><em>评分:<span>"&showscore(rsmark("score").value)&"</span></em></td><tr><td colspan=""2"" ><p><b>评:</b>"&rsmark("remark").value&"</p><p class=""modifyThisRemark""><a href=""admin/art-remark-modify.asp?ord="&rsmark("ord")&""" onclick=""modifyOrDelete(this);return false"" >修改</a><a href=""admin/art-remark-delete.asp?ord="&rsmark("ord")&""" onclick=""modifyOrDelete(this);return false"" >删除</a></p></td></tr>")
	rsmark.moveprevious
