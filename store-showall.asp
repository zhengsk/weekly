...
.showall_ul li span.s_contant{display:none;}
.showall_ul li .show_contant{display:block;background-color:#ecf5d3;margin:15px 5px 0 5px; padding:10px 20px 10px; height:100px; overflow:hide;cursor:pointer;font-size:12px; color:#182937; overflow:hidden;}
...
<div class="main">
  <div class="left">
	 ...
	 sql="select ID,title,tim,[s_contant] from store order by ID desc"
	...
	response.Write(" <ul class=""showall_ul"">")
	 for i=1 to rs.pagesize
	 	if i mod 2<>0 then
	 		response.Write("<li><a href=""store.asp?ID="&rs("ID")&""" name=""list"" ><span class=""title"">"&rs("title")&"</span><span class=""time"">"&left(rs("tim"),10)&"</span><span class=""s_contant"">"&rs("s_contant")&"</span></a></li>")
	 		rs.movenext
		else 
	 		response.Write("<li class=""libgcolor""><a href=""store.asp?ID="&rs("ID")&""" name=""list""><span class=""title"">"&rs("title")&"</span><span class=""time"">"&left(rs("tim"),10)&"</span><span class=""s_contant"">"&rs("s_contant")&"</span></a></li>")
	 		rs.movenext
		end if 
	 	if rs.eof=true then 
			exit for
	...
	<div class="newestarticle"><!------------------------------------------------------------------------热门文章--------->
    	<p class="newest">最新文章</p>
   		...
   		<!--#include file="store-newest.asp"--> <!--------------------------------------------左侧最新文章------------------------>	
        	</ul>								
    </div>
    <div class="hotarticle">
    <p class="hot">热门文章</p>
       	<ul>
   		<!--#include file="store-hot.asp"--> <!--------------------------------------------左侧最新文章------------------------>	
...
<h1 id="foot"> <span>Copyright @ 2008—2012 福农林大学人物周刊网</span> <em>www.fafurwzk.com </em> </h1>
	...
	<%	sub showpage(abspage)            '<!-----------------------------以下是导航分页的函数------------------------------------->
		...
		if cint(abspage)=<1 or abspage="" then
			...
			response.Write("<a href=""store-showall.asp?abspage=1"">首页</a>&nbsp;&nbsp;<a href=""store-showall.asp?abspage="&abspage-1&""">上一页</a>")
		end if 
		...
		if abspage >= pagcount then
			...
			response.Write("<a href=""store-showall.asp?abspage="&abspage+1&""">下一页</a>&nbsp;&nbsp;<a href=""store-showall.asp?abspage="&pagcount&""">尾页</a>")
		end if
...
<script type="text/javascript" language="javascript" defer="defer">
 ...
 function toIt()
 {
	 document.location.href="store-showall.asp?abspage="+ this.value;
 }
</script>
</body>
</html>
